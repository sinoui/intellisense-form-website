...
首先在`src/plugins/detail-page-extends`文件夹下创建`helloPermission`方法。
...
在`src/plugins/detail-page-extends`文件夹下，创建`helloExtends`方法，将`helloPermission`方法注册进去。
...
export default function getCustomPermission(
  ...
  //已办结的流程，张三登录可以对`用户反馈`字段进行编辑
  if (
    formValues?.workflowStatus === "已办结" &&
