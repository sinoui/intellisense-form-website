...
1. 创建页面：`${widow.location.pathname}#/dialog/form-app/${formDesignerId}/create-page`
2. 打开草稿详情页：`${widow.location.pathname}#/dialog/form-app/${formDesignerId}/draft-page?recordId=${数据id}`
3. 查看数据详情页`${widow.location.pathname}#/dialog/form-app/${formDesignerId}/detail-page?recordId=${数据id}`
