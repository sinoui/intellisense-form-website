...
下载并解压缩<BundleLink type="font" isAlpha>字体服务发布包</BundleLink>，如下所示：
...
```bash
mkdir -p /apps/sino-intellisense-form
cd /apps/sino-intellisense-form
wget https://sino-intellisense-form.oss-cn-beijing.aliyuncs.com/software/chrome-linux.zip
unzip chrome-linux.zip
chmod +x ./chrome-linux/chrome
cp -r ./chrome-linux /usr/local/chrome
ln -s /usr/local/chrome/chrome /usr/local/bin/
```
...
如果提示失败，应该是缺少依赖，请安装好依赖。如果您是离线环境，需要离线安装依赖包。请参见相关文档完成安装：

- [centos 离线安装 Google Chrome](https://blog.csdn.net/ancientapesman/article/details/118340178)
- [ubuntu 包查询站点](https://packages.ubuntu.com/)
...
请参考下面的文档完成字体安装： [如何给 Linux 安装新的字体](https://blog.csdn.net/ouyangzhenxin/article/details/82387632)。
