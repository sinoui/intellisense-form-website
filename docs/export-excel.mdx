...
import { exportExcel } from '@sinoform/plugin-sinoform-helpers';
...
我们通过`@sinoform/plugin-sinoform-helpers`提供的`exportExcel`辅助方法来实现导出 Excel 功能。此方法接收如下参数：
...
appSetting.listPageConfig.addButton({
  type: 'ALL_PAGE',
  formId: '*',
  render: React.lazy(() => import('./LeftButton')),
  pos: 'left',
});
```
