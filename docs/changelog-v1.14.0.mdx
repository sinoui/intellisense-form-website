...
- feat: 支持开始节点直接到结束节点的流程(强起流程)
- feat: 支持[表单上填写意见控件](#支持表单上填写意见控件)
- feat: 支持[弹窗子表单控件](#支持弹窗子表单控件)
- feat: 使用公式控制表单字段的可见性、可编辑性，关于公式的配置规则可参考[表单配置|公式规则](expression-rules)
- feat: 用户选择、部门选择拓展：[支持定制选定值的展示方式](#用户选择支持定制值的展示方式)
- feat: 单行文本、多行文本默认值支持插值表达式[插值表达式](interpolation-expression)
- feat: 说明文字支持插值表达式[插值表达式](interpolation-expression)
- feat: 支持文件存储定制 包括 mongodb、nfs、s3(http/https)与 http-api [使用说明](./config-file-store)
- feat: 添加 license 证书验证 [license 配置说明](./config-license)
- feat: 新增导出智能表单表结构的接口 详情请查看[智能表单配置数据整体迁移](./sinofrom-sinomatrix-import-export)
...
用户选择字段新增`是否显示部门信息`、`最高部门级别`、`显示顺序`来支持定制用户选择时可以显示用户相关部门信息的需求。
![用户选择配置](assets/changelog-v1.14.0/user-select-config.png)
![用户选择值定制渲染](assets/changelog-v1.14.0/user-select-renderer.png)

### 部门选择支持定制值的展示方式

部门选择字段新增`是否显示部门信息`、`最高部门级别`、`显示顺序`来支持定制部门选择时可以显示部门的上级部门相关信息。
![部门选择配置](assets/changelog-v1.14.0/dept-select-config.png)
![部门选择值定制渲染](assets/changelog-v1.14.0/dept-select-renderer.png)
