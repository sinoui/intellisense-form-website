...
上面的代码创建了一个继承自 `HTMLElement` 类的 `HelloButton` 类， `connectedCallback` 函数相当于组件的初始化方法，它调用 `render` 方法，在它的内部生成 HTML，以展现一个按钮。并通过高亮部分的代码给其内部的按钮添加点击事件监听：
...
const HelloButtonReactComponent = webComponentWrapper(helloButtonElementName);
...
```typescript title="src/plugins/flow-button-hello/HelloButton.ts" {2,29}
import $ from "jquery";
import "./HelloButton.css";
