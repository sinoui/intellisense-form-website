...
```sql title="同步已办数据-delete sql"
delete
from FLOW_READ
where FILETYPEID = '3396254c845548bdbbd1cd55973ac6f5'
  and DEPTID = '18043'
  and USERID = '1'
  and RECORDID = '619b8f49ec2ee8ac2f5b32ef';
```

```sql title="同步已办数据-insert sql"
insert
into FLOW_READ(HANDDONEURL,
               SYS_ID,
               CREATETIME,
               FILETYPEID,
               SUPERWORKFLOW,
...
```sql title="同步待办数据-delete sql"
delete
from FLOW_WRITE
where WORKFLOWINFOID = 'c49bbb22c698423eb7874e479aad9ee7';
```

```sql title="同步待办数据-insert sql"
insert
into FLOW_WRITE(SYS_ID,
                PREUSERNAME,
                PREWFLEVELID,
                CREATETIME,
                FILETYPEID,
